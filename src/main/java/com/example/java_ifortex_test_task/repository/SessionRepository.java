...
public interface SessionRepository extends JpaRepository<Session, Long> {
    @Query(value = """
        SELECT *
        FROM sessions
        WHERE device_type = :#{#deviceType.ordinal()}
    ...
    @Query(value = """
        SELECT s.id, CASE s.device_type  WHEN 1 THEN 0 WHEN 2 THEN 1 END AS device_type, s.ended_at_utc, s.started_at_utc, s.user_id
        FROM sessions s
